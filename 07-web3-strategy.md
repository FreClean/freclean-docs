...
FreClean's entire Web3 strategy fits in one sentence: **accept USDm payments, through CeloHT, verified the same way any other payment is verified.** Everything else - no token, no lending, no yield product, no proprietary chain - is a deliberate exclusion, not an oversight.

| In scope | Out of scope |
|---|---|
| USDm payments via CeloHT | A FreClean token |
